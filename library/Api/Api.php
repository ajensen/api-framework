...
require_once PATH_API . DS . "Error.php";
require_once PATH_API_PARSER .  DS . "Base.php";
require_once PATH_API_HOOK .  DS . "Base.php";
require_once PATH_API_SERVICE .  DS . "Base.php";
require_once PATH_API .  DS . "Request.php";
require_once PATH_API .  DS . "Response.php";
require_once PATH_API .  DS . "Logger.php";
...
abstract class Api {
	...
	/**
	 * Config object, set with default data
	 * @var array
	 */
	protected $config = array( 
							"auth_required" 		=> true,
							"service_path" 			=> "Service" . DS,
							"custom_path" 			=> "Api"
						);
	

	/**
	 * The parser used to generate the output string
	 * @var Api_Parser_Base
	 */
	protected $parser;
	...
	/**
	 * Hooks executed before executing service
	 * @var array
	 */
	private $_hooks = array(
						Api_Hook_Base::HOOK_BEFORE_SERVICE_EXECUTE => array("Api_Hook_MethodAllowed")
					);
	...
	final public function handle($request, $config=array()) {
		...
		$service_file_fullpath 							= $service_file_path . DS . $service_file_name;
		...
		// Do before hooks
		$this->executeHooks(Api_Hook_Base::HOOK_BEFORE_SERVICE_EXECUTE, array($ServiceObject));
	...
	final public function send(Api_Response $response) {  
		...
		// Modify parser
		$this->executeHooks(Api_Hook_Base::HOOK_MODIFY_PARSER, array($this->parser));
	...
	public function setType($type=self::TYPE_XML) {
		...
		if ( file_exists ( PATH . DS . $this->config['custom_path'] . DS . "Parser" . DS . $type . ".php") ) {
			
			$parser 									= PATH . DS . $this->config['custom_path'] . DS . "Parser" . DS . $type . ".php";
			$parser_class_name 							= $this->config['custom_path'] . "_Parser_" . $type;
		
		} else {

			$parser 									= PATH_API . DS . "Parser" . DS . $type . ".php";
	 ...
	 * 
	 * @param string $service - String service requested for in the url
	 * @return string
	 */	
	...
	/**
	 * Returns the path to where the services are saved
	 * 
	...
	/**
	 * Class factory used to load dynamic hooks
	 * 
	...
	/**
	 * Add a new hook
	 * 		   Place custom hooks in the "Hook" folder, or include the class if placed elsewhere
	 * 
	 * @param string $class_name
	 * @return void
	 */
	public function addHook($class_name, $type=Api_Hook_Base::HOOK_BEFORE_SERVICE_EXECUTE) {
	 ...
	 * 
	 * @example Converts a string like "new-file" to "newFile"
	 * 
