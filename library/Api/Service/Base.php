...
abstract class Api_Service_Base {
	

	/**
	 * Represents the name for this service
	 *			Used when calling this service, as well as displayed in log files	 
	 * @var string
	 */
	...
	final protected function connect()
	{
		...
		if ( USE_REDIS ) {
		  // start by connect to redis server
	...
	final public function shutdown() {
		...
		if ( $execution_time > ($this->_ttl + 0.01) ) {
			...
			$this->_api->getLogger()->log("Service timeout " . get_class($this) . " (" . $execution_time . " seconds)");
			$response['code'] = 504;
	...
	/**
	 * Return name of requested service
	 * 
	 * @return string
	 */
